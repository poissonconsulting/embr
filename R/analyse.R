...
#'
#' @details
#' For each cmdstanr engine, embr supplies a fixed set of arguments to the
#' underlying cmdstanr call (derived from `niters`, `nchains`, `nthin`, `seed`,
#' etc.). Any of these passed via `...` are dropped with a warning. The
#' remaining cmdstanr arguments can be passed through `...`. The lists below
#' name the embr-controlled arguments and highlight common pass-through
#' arguments; see the linked cmdstanr reference for the full set.
#'
#' `init` is the one embr-supplied argument that *can* be overridden via `...`,
#' which replaces inits generated by `gen_inits()` in [model()]. Non-MCMC
#' engines do not use `gen_inits()`.
#'
#' `nthin` and `niters_warmup` are unused by non-MCMC engines. For pathfinder,
#' variational, and laplace, `niters` is the number of samples drawn from the
