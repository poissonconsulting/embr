...
Three layered S3 class hierarchies, each with engine-specific subclasses (`jmb_*`, `smb_*`, `tmb_*`, `pmb_*`, `lmb_*`):
...
1. `select_rescale_data()` — selects columns per `select_data` spec, applies centering/scaling
2. `modify_data()` — converts data.frame to named list via user-supplied function + `numericize_factors()`
3. `analyse1()` (engine) — fits model, returns `mb_analysis`
4. `coef()`, `predict()`, `mcmc_derive()` — extract results from `mcmcr` posterior
